...
const BASE_URL = 'https://refuel-backend-5h8h.onrender.com';
...
module.exports = {
  ...
  async startCommand(ctx) {
    try {
      ...
      // Fetch token prices for ETH, BNB and MON
      const [ethPriceResponse, bscPriceResponse, monPriceResponse] = await Promise.all([
        ...
        api.getTokenDetails('0x760AfE86e5de5fa0Ee542fc7B7B713e1c5425701')
      ]).catch(error => {
      ...
      const monPrice = monPriceResponse.pairs?.length > 0 ? monPriceResponse.pairs[0].priceUsd.toFixed(2) : 'N/A';
  
      ctx.replyWithMarkdown(
        `👋 *Welcome ${firstName} to BikeBot Trading Bot!* \n\n` +
        `The *Fastest* ⚡ and most *Reliable* 🛡️ \n` +
        `🔍 Token Scanner \n` +
        `💱 Trading Bot \n\n` +
        `Paste 📝 any Token Contract Address on *ETH, BSC or MONAD* to Scan & Trade\n\n` +
        `*Market Prices:*\n` +
        `- ETH: $${ethPrice}\n` +
        `- BNB: $${bscPrice}\n` +
        `- MON: $${monPrice}\n\n` +
  ...
  async transferCommand(ctx) {
    try {
      ...
      const transferKeyboard = Markup.inlineKeyboard([
        ...
        [
          Markup.button.callback('Transfer MON', 'transfer_mon')
        ],
    ...
    try {
      ...
      for (const wallet of existingWallets) {
        ...
        try {
          ...
          const balances = await getWalletDisplay(wallet, ethProvider, bscProvider, monadProvider);
          message += `ETH Balance: ${Number(balances.ethBalance).toFixed(4)} ETH\n`;
          message += `BNB Balance: ${Number(balances.bscBalance).toFixed(4)} BNB\n`;
          message += `MON Balance: ${Number(balances.monadBalance).toFixed(4)} MON\n`;
        } catch (error) {
...
bot.action(['transfer_eth', 'transfer_bnb', 'transfer_mon'], async (ctx) => {
  try {
    await ctx.answerCbQuery();
    const coinMap = {
      'transfer_eth': 'ETH',
    ...
    const coin = coinMap[ctx.match[0]];
    ...
    const providerMap = {
      ...
      'MON': process.env.MONAD_RPC_URL
    };
    const provider = new ethers.JsonRpcProvider(providerMap[coin]);
...
bot.action(/wallet_transfer_(eth|bnb|mon)_wallet(\d)/, async (ctx) => {
  try {
    await ctx.answerCbQuery();
    ...
    const provider = new ethers.JsonRpcProvider(providerMap[upperCoin]);
...
bot.action('cancel_transfer', async (ctx) => {
  ...
  async getWalletDisplay(wallet, ethProvider, bscProvider, monadProvider) {
    try {
      ...
      return {
        ...
        monadBalance: ethers.formatEther(monadBalance)
      };
    } catch (error) {
