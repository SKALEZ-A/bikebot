...
async function formatBriefResponse(data, address, chainName, userId, tradeMode = 'buy', currentWallet = 'wallet1') {
  try {
    ...
    try {
      const response = await axios.get(`${BASE_URL}/wallet/evm/settings/${userId}/${currentWallet}`);
      if (response.data && response.data.settings) {
        settings = response.data;
      }
    } catch (error) {
      console.log('Using default settings due to API error:', error.message);
    }

    const tokenName = data.baseToken?.name || 'Unknown Token';
    const tokenSymbol = data.baseToken?.symbol || '???';
    ...
    message += `💧 Liquidity: $${formatNumberWithCommas(liquidity)}\n`;
    message += `📊 24h Volume: $${formatNumberWithCommas(volume)}\n\n`;
...
function createChainButtons() {
    return Markup.inlineKeyboard([
        [
            Markup.button.callback('ETH', 'chain_eth'),
            Markup.button.callback('BSC', 'chain_bsc'),
            Markup.button.callback('MON', 'chain_monad')
        ]
    ]);
}

// Add chain handler
bot.action('chain_monad', async (ctx) => {
    try {
        ctx.session.selectedChain = 'monad';
        await ctx.editMessageText(
            'Selected chain: Monad Testnet (MON)',
            Markup.inlineKeyboard([[Markup.button.callback('Change Chain', 'change_chain')]])
        );
    } catch (error) {
        console.error('Error handling chain selection:', error);
        await ctx.reply('Error selecting chain. Please try again.');
    }
});
